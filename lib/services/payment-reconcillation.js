...
export class PaymentReconciliationService {
  ...
  static async handleWebhook(webhookData) {
    try {
      ...
      if (status !== "successful") {
        console.log("Non-successful payment webhook received:", status);
  ...
  static async processUnlockWithRetry(
    ...
    try {
      console.log(`Unlock attempt ${attempt}/${maxRetries} for ${txRef}`);
      ...
      if (data.success) {
        console.log("Unlock successful:", txRef);
  ...
  static async queueForManualReconciliation(transactionId, txRef, error) {
    try {
      ...
      console.log("Queued for manual reconciliation:", txRef);
    } catch (err) {
  ...
  static async runReconciliationJob() {
    try {
      ...
      for (const payment of recentPayments) {
        ...
        if (!isUnlocked) {
          console.log("Found unprocessed payment:", payment.tx_ref);
