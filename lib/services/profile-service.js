...
export class ProfileService {
  ...
  static async getUserProfile(userId) {
    try {
      // Run all three queries in parallel — one network round-trip instead of three
      const [studentResult, weeksResult, paymentsResult] = await Promise.all([
        supabase.from("students").select("*").eq("user_id", userId).single(),
        supabase
      ...
      const { count: unlockedCount } = weeksResult;
  ...
  static async getLecturers() {
    try {
      ...
      // Resolve storage paths → public URLs for the "lecturers" bucket
      return (data || []).map((lecturer) => {
        if (lecturer.image_url && !lecturer.image_url.startsWith("http")) {
          const { data: urlData } = supabase.storage
            .from("lecturers")
            .getPublicUrl(lecturer.image_url);
          return { ...lecturer, image_url: urlData?.publicUrl || null };
        }
        return lecturer;
      });
    } catch (error) {
  ...
  /**
   * Get all published week titles in one query (for curriculum grid)
   */
  static async getAllWeekTitles() {
    try {
      const { data, error } = await supabase
        .from("week_content")
      ...
      if (error) throw error;
      // Return as a map: { weekNumber: title }
      return (data || []).reduce((acc, row) => {
        acc[row.week_number] = row.title;
        return acc;
      }, {});
    } catch (error) {
      console.error("Error fetching week titles:", error);
      return {};
    }
  }
