...
export class PaymentService {
  ...
  static async initializePayment(registrationData, retryCount = 0) {
    try {
      console.log(
        `[PaymentService] Initializing payment (attempt ${retryCount + 1}/${
          MAX_RETRIES + 1
        })`
      );
      ...
      if (!response.ok) {
        ...
        try {
          ...
          // If parsing fails, check if we should retry
          if (retryCount < MAX_RETRIES && response.status >= 500) {
            console.log(
              `[PaymentService] Server error, retrying in ${RETRY_DELAY}ms...`
            );
      ...
      console.log("[PaymentService] Payment initialized successfully");
      return data.data;
    } catch (error) {
      ...
      if (
        ...
        console.log(
          `[PaymentService] Network error, retrying in ${RETRY_DELAY}ms...`
        );
  ...
  static async verifyPayment(transactionId, retryCount = 0) {
    try {
      console.log(
        `[PaymentService] Verifying payment ${transactionId} (attempt ${
          retryCount + 1
        })`
      );
      ...
      const responseText = await response.text();
      console.log(
        `[PaymentService] Verify response status: ${response.status}`
      );
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (retryCount < MAX_RETRIES && response.status >= 500) {
            console.log(
              `[PaymentService] Server error, retrying verification...`
            );
            await delay(RETRY_DELAY * (retryCount + 1));
      ...
      if (
        ...
        console.log(`[PaymentService] Network error, retrying verification...`);
        await delay(RETRY_DELAY * (retryCount + 1));
        return this.verifyPayment(transactionId, retryCount + 1);
      }
  ...
  static async registerWithPayment(paymentId, retryCount = 0) {
    try {
      console.log(`[PaymentService] Registering with payment ${paymentId}`);
      ...
      console.log("[PaymentService] Registration successful");
      return data.data;
    } catch (error) {
      console.error("[PaymentService] Registration error:", error);
